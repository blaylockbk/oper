...
def draw_fires_on_map():
    ...
    bot_left_lat  = np.nanmin(fires['LAT'])-5
    bot_left_lon  = np.nanmin(fires['LONG'])-5
    top_right_lat = np.nanmax(fires['LAT'])+5
    top_right_lon = np.nanmax(fires['LONG'])+5
    m = Basemap(resolution='i', projection='cyl', area_thresh=1000,\
