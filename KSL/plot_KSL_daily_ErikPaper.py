...
Might be more intuitive to save the figure in local time rather than UTC time
...
def plot_pm25_timeseries(mobile):
    ...
    # Left axis: PM 2.5 Concentration
    line1 = ax1.plot(mobile['DATES'],mobile['pm25'],color='orangered',linewidth=lw,label="PM 2.5 Salt Lake Valey",zorder=2)
    line2 = ax1.plot(mobile['DATES'][mobile['latitude']<40.48],mobile['pm25'][mobile['latitude']<40.48],color='violet',linewidth=lw,label="PM 2.5 Utah Valley",zorder=2)
    ax1.set_ylabel(r'PM 2.5 ($\mu$g m$\mathregular{^{-3}}$)',fontsize=label_font)
    ax1.set_xlabel('Time (UTC)',fontsize=label_font)
    ax1.set_yticks(range(0,61,10))
    ax1.set_xlim([datetime(2016,2,9,23,20),datetime(2016,2,10,1,40)])
    #ax1.legend(bbox_to_anchor=(0, 1.0, .45, .12),fontsize=7,frameon=False)
    plt.grid(linewidth=.25,linestyle=':')
    
    # Right axis: Elevation
    ax1a = ax1.twinx()
    line3 = ax1a.plot(mobile['DATES'],mobile['elevation'],color='teal',lw=lw-.3,label='Elevation',zorder=1)
    ax1a.set_ylabel('Elevation (m)',fontsize=label_font)
    ax1a.set_yticks(range(1500,3501,500))
    #ax1a.set_ylim([1200,4700])
    #ax1a.legend(bbox_to_anchor=(0, 1.0,.8, .12),fontsize=7,frameon=False)
...
def plot_ozone_theta_scatter(mobile):
    ...
    ax1a = ax1.twiny()
    ax1a.plot(theta,height_asl,color='red',lw=lw,label='Potential Temperature')
...
def plot_pm25_theta_scatter(mobile):
    ...
    ax1.scatter(mobile['pm25'][mobile['latitude']<40.48],mobile['elevation'][mobile['latitude']<40.48],color='violet',s=3)
    ax1.set_ylabel('Elevation (m)',fontsize=label_font)
    ax1.set_xlabel(r'PM 2.5 ($\mu$g m$\mathregular{^{-3}}$)',fontsize=label_font)
    ax1.set_xlim([0,70])
    #ax1.set_yticks(range(1200,4701,500))
...
def plot_ozone_map(mobile,other_mobile=False,auto_map_boundaries=True,background='WRF'):
    ...
    if other_mobile == True:
        ...
        try:
            ...
            m.scatter(x[:],y[:],c=trax_mobile['ozone'][:],vmax=80,vmin=45.,
                      lw = .3,marker='d',s=30,cmap=plt.cm.get_cmap('jet'),
                      edgecolor='k',zorder=50)
        except:
        ...
        try:
            ...
            m.scatter(x[:],y[:],c=trk3_mobile['ozone'][:],vmax=80,vmin=45.,
                      edgecolor='k',lw = .3,marker='p',s=30,cmap=plt.cm.get_cmap('jet'),zorder=50)
        except:
            print 'no UUTK3 data'
        ...
        try:
            ...
            m.scatter(x[:],y[:],c=trk1_mobile['ozone'][:],vmax=80,vmin=45.,
                      edgecolor='k',lw = .3,marker='p',s=30,cmap=plt.cm.get_cmap('jet'),zorder=50)
        except:
            print 'no UUTK1 data'
    ...
    # ozone
    m.scatter(a['LON'],a['LAT'],c=a['OZONE'],vmax=80,vmin=45.,
              edgecolor='k',
              lw =.3,s=20, marker='s',cmap=plt.cm.get_cmap('jet'),zorder=50)
    ...
    m.scatter(x[::3],y[::3],c=ksl_mobile['ozone'][::3],
              edgecolor='k',
              vmax=80,vmin=45.,lw = .3,s=30,cmap=plt.cm.get_cmap('jet'),zorder=50)
...
if __name__ == '__main__':
    ...
    cases = {
            ...
            'ErikLakeBreeze':[datetime(2016, 6, 28, 0, 0), 120],
            'ErikAntIsFire':[datetime(2016, 7, 23, 16, 30), 120],
            'ErikWinter':[datetime(2016, 2, 10, 1), 100]
    }
    ...
    time_break_locations = np.argwhere(ksl_deltat>timedelta(0,80)) # if time between obs is greater than 50 minutes than it's likely a new flight path
    ...
    for i in time_break_locations:
        ...
        if np.nanmax(ksl_mobile['ozone'])>0:
            ...
            list_of_files+=plot_ozone_map(ksl_mobile,other_mobile=False,background='topo')
            list_of_files+='\n'
        if np.nanmax(ksl_mobile['pm25'])>0:
            ...
            list_of_files+=plot_pm25_map(ksl_mobile,other_mobile=False,background='topo')
            list_of_files+='\n'
