...
if __name__ == '__main__':
    ...
    request_time = datetime(2015,6,1,0)
    request_time = datetime(2017,1,1,0)
    one_day = 1500    
    
    this_day = request_time
    while this_day <= datetime.now():
        print '**----------------------------------------------------------------------------'        
        print 'plotting', this_day, 'and previous', one_day, 'minutes'
        print '**----------------------------------------------------------------------------'       
        
        # Get Mobile Data sets if they are available
        ksl_day = download_mobile('KSL5',this_day,one_day)           # KSL Helicopter
        if ksl_day == 'No Data Available from KSL5 at this time':
            list_of_files+=this_day.strftime('%d %b %Y ending %H:%M UTC')+'\n'
            print "!! NOT AVAILALBE !!"
            this_day = this_day +timedelta(days=1)            
            continue
        
        print 'range:',ksl_day['DATES'][0],ksl_day['DATES'][0]        
        ...
        for i in time_break_locations:
            ...
            try:                
                ...
                print "!! There might not be KSL data even though it we think there is!!"
