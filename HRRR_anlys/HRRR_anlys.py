# Brian Blaylock
# May 19, 2017                             Saw Kristin Chenoweth perform at BYU
...
for n in locs_idx:
    ...
    if fxx == 0:
        plt.suptitle('HRRR Analysis: %s' % (l['name']), y=1)
    else:
        plt.suptitle('HRRR f%s: %s' % (fxx, l['name']), y=1)
...
for hh in range(len(TS_dates)):
    ...
    for n in locs_idx:
        ...
        #
        # 4) Save figure
        figs[locName][0].savefig(SAVE+'h%02d.png' % hh)
        #
        pntTemp.remove()
        pntDwpt.remove()
        pntGust.remove()
